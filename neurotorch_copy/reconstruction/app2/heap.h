...
class BasicHeap {     // Basic Min heap
  ...
  private:
    ...
    bool swap_heap(int id1, int id2) {
        if(id1 < 0 || id1 >= elems.size() || id2 < 0
                || id2 >= elems.size()) return false;
...
class Heap {
  ...
  private:
    vector<T1> elems;
    T2 * vals;
    bool swap_heap(int id1, int
                   id2) { // swap id1 and id2 if elems[id1] and elems[id2] doesn't fit heap model
        if(id1 < 0 || id1 >= elems.size() || id2 < 0
                || id2 >= elems.size()) return false;
        if(id1 == id2) return false;
        int pid = id1 < id2 ? id1 : id2;

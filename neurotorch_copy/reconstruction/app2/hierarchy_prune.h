...
template<class T> bool swc2topo_segs(vector<MyMarker*> & inswc,
                                     vector<HierarchySegment*> & topo_segs,
                                     int length_method = INTENSITY_DISTANCE_METHOD, T * inimg1d = 0, long sz0 = 0,
                                     long sz1 = 0, long sz2 = 0) {
    if(length_method == INTENSITY_DISTANCE_METHOD && (inimg1d == 0 || sz0 == 0
            || sz1 == 0 || sz2 == 0)) {
    ...
    map<MyMarker*, V3DLONG> swc_map;
    for(V3DLONG i = 0; i < tol_num; i++) swc_map[inswc[i]] = i;
    ...
    vector<V3DLONG> childs_num(tol_num);
    {
        ...
        for(V3DLONG i = 0; i < tol_num;
                i++) if(childs_num[i] == 0) leaf_markers.push_back(inswc[i]);
    }
    ...
    for(V3DLONG i = 0; i < leaf_num; i++) {
        ...
        topo_dists[cid] = (length_method == INTENSITY_DISTANCE_METHOD) ?
                          inimg1d[leaf_marker->ind(sz0, sz01)]/255.0 : 0;
        while(parent_node) {
            V3DLONG pid = swc_map[parent_node];
            double tmp_dst = (length_method == INTENSITY_DISTANCE_METHOD) ?
                             (inimg1d[parent_node->ind(sz0,
                                      sz01)]/255.0 + topo_dists[cid]) : (dist(*child_node,
                                              *parent_node) + topo_dists[cid]);
...
template<class T>  bool topo_segs2swc(vector<HierarchySegment*> & topo_segs,
    ...
    max_dst -= min_dst;
    if(max_dst == 0.0) max_dst = 0.0000001;
    max_level -= min_level;
    if(max_level == 0) max_level = 1.0;
    for(V3DLONG i = 0; i < topo_segs.size(); i++) {
        double dst = topo_segs[i]->length;
        int level = MIN(topo_segs[i]->level, max_level);    // todo1
        int color_id = (swc_type == 0) ? (dst - min_dst) / max_dst * 254.0 + 20.5 :
...
template<class T> bool hierarchy_prune(vector<MyMarker*> &inswc,
                                       vector<MyMarker*> & outswc, T * inimg1d, long sz0, long sz1, long sz2,
...
template<class T> bool happ(vector<MyMarker*> &inswc,
                            vector<MyMarker*> & outswc, T * inimg1d, long sz0, long sz1, long sz2,
    ...
    cout<<"pruned by length_thresh (segment number) : "<<topo_segs.size()
        <<" - "<<topo_segs.size() - filter_segs.size()<<" = "<<filter_segs.size()<<endl;
    ...
    // calculate radius for every node
    {
        ...
        for(V3DLONG i = 0; i < filter_segs.size(); i++) {
            ...
            while(true) {
                ...
                if (real_thres<bkg_thresh) real_thres = bkg_thresh; //by PHC 20121012
...
#endif
    {
        ...
        while(it != seg_dist_map.rend()) {
            ...
            if(!seg->parent || sum_rdc == 0.0 || (sum_sig/sum_rdc >= SR_RATIO
                                                  && sum_sig >= 1.0 * T_max)) {
                tol_sum_sig += sum_sig;
                tol_sum_rdc += sum_rdc;
                ...
                MyMarker * p = leaf_marker;
                while(true) {
                    if(tmpimg1d[p->ind(sz0, sz01)] != 0) seg_markers.push_back(p);
                    if(p == root_marker) break;
                    p = p->parent;
                }
        ...
        cout<<"prune by coverage (segment number) : "<<seg_dist_map.size() << " - "<<
            filter_segs.size() <<" = "<<seg_dist_map.size() - filter_segs.size()<<endl;
        cout<<"R/S ratio = "<<tol_sum_rdc/tol_sum_sig<<" ("<<tol_sum_rdc<<"/"<<tol_sum_sig<<")"<<endl;
        if(1) { // evaluation
            double tree_sig = 0.0;
            for(int m = 0; m < inswc.size();
                    m++) tree_sig += inimg1d[inswc[m]->ind(sz0, sz01)];
            double covered_sig = 0.0;
            for(int ind = 0; ind < tol_sz;
                    ind++) if(tmpimg1d[ind] == 0) covered_sig += inimg1d[ind];
    ...
    if(0) { // resampling markers or internal node pruning //this part of code has bug: many fragmentations. noted by PHC, 20120628
        ...
        for(V3DLONG i = 0; i < filter_segs.size(); i++) {
            ...
            if((*sampling_markers.rbegin()) != leaf_marker) sampling_markers.push_back(
                    leaf_marker);
            for(int m = 1; m < sampling_markers.size();
                    m++) sampling_markers[m]->parent = sampling_markers[m-1];
        }
    }
...
#endif
    {
        ...
        if(1) { // get child_num of each node
            ...
            for(int m = 0; m < current_markers.size();
                    m++) tmp_child_num[current_markers[m]] = 0;
                ...
                for(V3DLONG kk = -r1; kk <= r1; kk++) {
                    ...
                    for(V3DLONG jj = -r1; jj <= r1; jj++) {
                        ...
                        for(V3DLONG ii = -r1; ii <= r1; ii++) {
                            ...
                            if((z2 - zz) * (z2 - zz) + (y2 - yy) * (y2 - yy) + (x2 - xx) *
                                    (x2 - xx) <= r2 * r2) {
...
#endif
    {
        ...
        if(1) { // start prune leaf nodes
            ...
            for(V3DLONG i = 0; i < filter_segs.size(); i++) {
                ...
                MyMarker * root_marker = seg->root_marker;
                if(leaf_marker
                        && root_marker) tmp_segs.push_back(seg); // filter out empty segments
            }
            cout<<"\t"<<filter_segs.size() - tmp_segs.size()
                <<" hierarchical segments are pruned in joint leaf node pruning"<<endl;
            filter_segs.clear();
            filter_segs = tmp_segs;
        }

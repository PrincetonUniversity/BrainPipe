    ...
    while(ifs.good()) {
        ...
        if(0 && marker_set.find(marker) != marker_set.end()) {
            cout<<"omit duplicated marker"<<markers.size()
                <<" : x = "<<marker.x<<" y = "<<marker.y<<" z = "<<marker.z<<" r = "<<marker.radius<<endl;
        } else {
            markers.push_back(marker);
            marker_set.insert(marker);
            if(0) cout<<"marker"<<markers.size()
                          <<" : x = "<<marker.x<<" y = "<<marker.y<<" z = "<<marker.z<<" r = "<<marker.radius<<endl;
        }
        count++;
    }
...
bool saveMarker_file(string marker_file, vector<MyMarker> & outmarkers,
    ...
    for(int i = 0; i < outmarkers.size(); i++) {
        ofs<<outmarkers[i].x + MARKER_BASE<<","<<outmarkers[i].y +
           MARKER_BASE<<","<<outmarkers[i].z +
           MARKER_BASE<<","<<outmarkers[i].radius<<endl;
    }
    ofs.close();
    return true;
}
...
bool saveSWC_file(string swc_file, vector<MyMarker*> & outmarkers,
                  list<string> & infostring) {
    if(swc_file.find_last_of(".dot") == swc_file.size() - 1) return saveDot_file(
                    swc_file, outmarkers);
...
bool saveSWC_file(string swc_file, vector<NeuronSWC*> & outmarkers,
                  list<string> & infostring) {
    cout<<"marker num = "<<outmarkers.size()<<", save swc file to "<<swc_file<<endl;
    ofstream ofs(swc_file.c_str());
...
bool readESWC_file(string swc_file, vector<MyMarkerX*> & swc) {
    ...
    while(ifs.good()) {
        ...
        if(my_id == -1) break;
        if(marker_map.find(my_id) != marker_map.end()) {
            cerr<<"Duplicate Node. This is a graph file. Please read it as a graph."<<endl;
            return false;
        }
        marker_map[my_id] = marker;
...
vector<MyMarker*> getLeaf_markers(vector<MyMarker*> & inmarkers) {
    ...
    for(int i = 0; i < inmarkers.size(); i++) {
        if(par_markers.find(inmarkers[i]) == par_markers.end()) leaf_markers.push_back(
                inmarkers[i]);
    }
    par_markers.clear();
    return leaf_markers;

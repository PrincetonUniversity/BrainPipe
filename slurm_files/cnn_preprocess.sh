...
OUT0=$(sbatch slurm_scripts/cnn_step0.sh "`pwd`") 
echo $OUT0

#generate chunks for cnn input
OUT1=$(sbatch --dependency=afterany:${OUT0##* } --array=0-130 slurm_scripts/cnn_step1.sh "`pwd`") 
echo $OUT1

#check if correct number of patches were made
OUT2=$(sbatch --dependency=afterany:${OUT1##* } slurm_scripts/cnn_step1_check.sh "`pwd`") 
echo $OUT2

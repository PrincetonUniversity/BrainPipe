...
OUT0=$(sbatch slurm_scripts/cnn_step21.sh "`pwd`") 
echo $OUT0

#populate reconstructed array
OUT1=$(sbatch --dependency=afterany:${OUT0##* } --array=0-130 slurm_scripts/cnn_step2.sh "`pwd`") 
echo $OUT1

#generate cell measures
OUT2=$(sbatch --dependency=afterany:${OUT1##* } --array=0-30 slurm_scripts/cnn_step3.sh "`pwd`") 
echo $OUT2

#export csv dictionary and run last check
OUT3=$(sbatch --dependency=afterany:${OUT2##* } slurm_scripts/cnn_step4.sh "`pwd`") 
echo $OUT3

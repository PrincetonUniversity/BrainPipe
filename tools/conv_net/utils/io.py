...
def consolidate_cell_measures_bulk(pth):
    ...
    fls = [xx for xx in os.listdir(
        pth) if "reconstructed_array.npy" in os.listdir(os.path.join(pth, xx))]
...
def make_inference_output_folder(pth):
    ...
    if not os.path.exists(os.path.join(pth, "output_chnks")):
        os.mkdir(os.path.join(pth, "output_chnks"))
...
def pairwise_distance_metrics_given_cdists(
    ...
    start = time.time()
    if verbose:
        print("\nCalculating pairwse distances at cutoff {}...".format(cutoff))
    st = time.time()
    ...
    if verbose:
        print("  {} seconds for calculating and collecting distances".format(
            np.round(time.time() - st, decimals=3)))
    ...
    if verbose:
        sys.stdout.write(
            "  Sorted in {} seconds.".format(np.round(
                time.time() - st, decimals=3)))
        sys.stdout.flush()
    ...
    for (i, dist) in enumerate(dists):
        ...
        if d > cutoff:
            ...
            if verbose:
                print(
                    "Reached cutoff distance, so far {} paired".format(
                        len(paired)))
            break
        truth_idx = dist[1]
        pred_idx = dist[2]
...
def resize(pth, dst, resizef=6):
    ...
    if pth[-4:] == ".tif":
        ...
        img = np.lib.format.open_memmap(pth, dtype="float32", mode="r")

    z, y, x = img.shape

    for i in range(z):
        # make the factors
        xr = img[i].shape[1] / resizef
        yr = img[i].shape[0] / resizef
        im = cv2.resize(img[i], (xr, yr), interpolation=cv2.INTER_LINEAR)
        tifffile.imsave(os.path.join(dst, "zpln{}.tif".format(
            str(i).zfill(12))), im.astype("float32"), compress=1)
...
def resize_stack(pth, dst):
    ...
    # get all tifs
    fls = [os.path.join(pth, xx) for xx in os.listdir(pth) if xx[0] !=
           "." and "~" not in xx and "Thumbs.db" not in xx and ".tif" in xx]
    fls.sort()
...
def sample_reconstructed_array(pth, zstart, zend):
    ...
    if "reconstructed_array.npy" in flds:
        ...
        chunk = np.lib.format.open_memmap(os.path.join(
            pth, "reconstructed_array.npy"), dtype="float32", mode="r")
        print(chunk.shape)

...
from tools.conv_net.utils.io import listdirfull, load_np
from tools.conv_net.utils.io import makedir, save_dictionary
import os
import sys
import numpy as np
...
def basic_convert(tf, zp, saveLocation):
    ...
    if np.all((os.path.exists(tf), os.path.exists(zp))):
        generate_mem_mapped_array_for_net_training(
            impth=tf, roipth=zp, dst=os.path.join(
                saveLocation, os.path.basename(tf)[:-4]+".npy"),
            verbose=True)
    else:
        sys.stdout.write(
...
def convert_input(inputFolder, saveLocation, remove_bad=True):
    """Function for converting data from imageJ ROIs +
    data to mem_mapped arrays for preprocessing + batch
    generation to pass to cnn
    """
    # get pairs
    ...
    # save out points
    save_dictionary(os.path.join(os.path.dirname(saveLocation),
                                 "points_dictionary.p"), file_points_dct)
...
def generate_mem_mapped_array_for_net_training(impth, roipth, dst, verbose=True):
    """Function for generating a memory mapped array given path to
    imagestack and roi zip file

    To load array after use:
        np.lib.format.open_memmap(pth_to_array, dtype = "uint16",
        mode = "r")
    ...
    annotation_type =
                    "centers": use if centers of cells were marked
                    using FIJI"s point tool
    ...
    assert all((impth[-4:] == ".tif", np.any((roipth[-4:] == ".zip", roipth[-4:] == ".roi")), dst[-4:] ==
                ".npy")), "impth, roipth, and dst paths must be strings ending .tif, .zip, and .npy respectively"
    ...
    if ".zip" in roipth:
        ...
        rois_formatted = list(zip(*[map(int, xx.replace(".roi", "").split("-")[0:3])
                                    for xx in rois if len(xx.split("-")) == 3]))
    else:
    ...
    if len(rois_formatted) == 0:
        ...
        rois_formatted = list(zip(*[map(int, xx.replace(".roi", "").split("-")[0:3])
                                    for xx in rois if len(xx.split("-")) == 4]))
...
def otsu_par(saveLocation, otsufld, size, otsu_factor):
    ...
    p = mp.Pool(12)
    iterlst = [(
        otsufld, inn, size, otsu_factor) for inn in listdirfull(
            saveLocation, "npy")]
...
def otsu_helper(otsufld, inn, size, otsu_factor):
    ...
    # save input
    tifffile.imsave(os.path.join(otsufld, "{}_img.tif".format(
        os.path.basename(inn)[:-4])), raw.astype("float32"))

    # save input
    otsu = otsu_dilate(raw, lbl, size=size, otsu_factor=otsu_factor)
    tifffile.imsave(os.path.join(otsufld, "{}_lbl.tif".format(
        os.path.basename(inn)[:-4])), otsu.astype("float32"))
...
def otsu_dilate(arr0, arr1, size=(8, 60, 60), otsu_factor=0.8):
    ...
    arr1=points
    size=(z,y,x)
    otsu_factor - scaling of the otsu value, >1 is less stringent,
    ...
    for pnt in pnts:
        ...
        vol[vol < v] = 0
        vol[vol >= v] = 1
        nvol = np.maximum(outarr[np.max((pnt[0]-size[0], 0)):pnt[0]+size[0], np.max(
            (pnt[1]-size[1], 0)):pnt[1]+size[1], np.max((pnt[2]-size[2], 0)):pnt[2]+size[2]], vol)
...
if __name__ == "__main__":
    ...
    saveLocation = "/tigress/ejdennis/cnn/arrays"
    makedir(saveLocation)
    otsufld = "/tigress/ejdennis/cnn/otsu"
    makedir(otsufld)
    size = (5, 10, 10)
    otsu_factor = 0.8

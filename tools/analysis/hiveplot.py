...
if __name__ == '__main__':
    ...
    structures = make_structure_objects(
        excelfl, remove_childless_structures_not_repsented_in_ABA=True, ann_pth=ann_pth, verbose=True)
...
def make_hiveplot(inputexcel, outputexcel, node1='Cerebellum', node2='Thalamus', node3=False, colorlst=['black', 'red', 'white'], title=False, annpath=False, threshold=0.25):
    ...
    innstructures = make_structure_objects(
        inputexcel, remove_childless_structures_not_repsented_in_ABA=True, ann_pth=ann_pth)
    outstructures = make_structure_objects(
        outputexcel, remove_childless_structures_not_repsented_in_ABA=True, ann_pth=ann_pth)

    # make networkx graphs
    Ginn = make_network(innstructures, substructure_name='Basic cell groups and regions',
                        graphstyle='twopi', showcellcounts=False, cutofflevel=10, font_size=11, show=False)
    Gout = make_network(outstructures, substructure_name='Basic cell groups and regions',
                        graphstyle='twopi', showcellcounts=False, cutofflevel=10, font_size=11, show=False)
    ...
    if node3:
        nn3 = [xx for xx in outstructures if xx.name == node3][0]

    # pull out progeny of nodes; might need to make this level based
    nodes = dict()
    nodes['group1'] = [(n, d['structure_object'].cellcount_progeny) for n, d in Ginn.nodes(
        data=True) if d.values()[1].name in [str(xx[1]) for xx in nn1.progeny]]
    nodes['group2'] = [(n, d['structure_object'].cellcount_progeny) for n, d in Gout.nodes(
        data=True) if d.values()[1].name in [str(xx[1]) for xx in nn2.progeny]]
    if node3:
        nodes['group3'] = [(n, d['structure_object'].cellcount_progeny) for n, d in Gout.nodes(
            data=True) if d.values()[1].name in [str(xx[1]) for xx in nn3.progeny]]
    ...
    if node3:
        n3lst = [[i, [xx.cellcount for xx in outstructures if xx.name == i][0]]
    ...
    edges = dict()
    edges['group1'] = [(xx[0], yy[0], (((xx[1]/n1max)+(yy[1]/n2max))/2))
                       for xx in n1lst for yy in n2lst if xx[1] > 0 and yy[1] > 0 and (((xx[1]/n1max)+(yy[1]/n2max))/2) > threshold]
    if node3:
        edges['group2'] = [(xx[0], yy[0], (((xx[1]/n1max)+(yy[1]/n3max))/2))
                           for xx in n1lst for yy in n3lst if xx[1] > 0 and yy[1] > 0 and (((xx[1]/n1max)+(yy[1]/n3max))/2) > threshold]
    ...
    if node3:
        nodes_cmap['group3'] = colorlst[2]

    edges_cmap = dict()
    edges_cmap['group1'] = (0, 0.5, 0.5)
    if node3:
        edges_cmap['group2'] = (0.2, 0.4, 0.9)
    ...
    h = HivePlot(nodes, edges, nodes_cmap, edges_cmap,
                 is_directed=True, fig=f, linewidth=0.3, ax=ax)
    if title:
        f.suptitle(title, fontsize=14, fontweight='bold')
    legend = [mpatches.Patch(color=colorlst[0], label=node1),
              mpatches.Patch(color=colorlst[1], label=node2)]
    if node3:
        legend.append(mpatches.Patch(color=colorlst[2], label=node3))
    f.text(0.05, 0.2, '{} --> {}'.format(node1, node2),
           color=edges_cmap['group1'], backgroundcolor=(0.6, 0.6, 0.6), fontsize=14)
    if node3:
        f.text(0.05, 0.15, '{} --> {}'.format(node1, node3),
               color=edges_cmap['group2'], backgroundcolor=(0.6, 0.6, 0.6), fontsize=14)
    h.draw()

...
def make_info_file(volume_size, resolution, layer_dir, commit=True, atlas_type=None):
    ...
    vol.provenance.owners = ['ejdennis@princeton.edu']  # list of contact email addresses
    if commit:
...
def make_downsample_tasks(vol, mip_start=0, num_mips=3):
    ...
    tasks = tc.create_downsampling_tasks(
        ...
        # manually set chunk size of next scales, overrides preserve_chunk_size
...
if __name__ == "__main__":
    ...
    # 1.1x data
    step = sys.argv[1]
    viz_dir = '/jukebox/brody/lightsheet/neuroglancer'
    ...
    """ Make progress dir """
    progress_dir = mkdir(
        viz_dir + f'/progress_{layer_name}')  # unlike os.mkdir doesn't crash on prexisting

...
if __name__ == "__main__":
    ...
    transformfiles = ["/elastix_inverse_transform/TransformParameters.0.txt",  # this is auto = fixed image; atlas = moving image
                      ...
                      "/jukebox/LightSheetTransfer/kelly/2020_07_15/20200715_12_14_06_f37080_mouse2_20171015/elastix_inverse_transform/reg_to_sig785/TransformParameters.0.txt",
...
# %%
    ...
    point_lst = transformed_pnts_to_allen_helper_func(np.load(os.path.join(
        dst, "posttransformed_zyx_voxels.npy")), tifffile.imread(annpth), order="ZYX")
    df = count_structure_lister(idtable, *point_lst).fillna(0)
    df.drop(columns="Unnamed: 0").to_csv(os.path.join(dst, os.path.basename(
        idtable).replace(".xlsx", "")+"_f37080_mouse2_20171015.csv"), index=None)

# %%
    # Code for converting .npy to .mat
    f = "/some/path/.npy"  # full path to the .npy that you want converted
    # full path to where the .mat file should be saved and what it should be named
    new_path = "some_other_path.mat"

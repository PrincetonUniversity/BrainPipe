...
def terastitcher_wrapper(**kwargs):
    ...
    if "labeltype" in kwargs and kwargs["labeltype"] == "rat":
        ...
        left = [os.path.join(src, xx) for xx in os.listdir(src) if "C00" in xx]
        left.sort()
        right = [os.path.join(src, xx) for xx in os.listdir(src) if "C01" in xx]
        right.sort()
    ...
    # for other mouse/high res images
        ...
        # blend lighsheets
        sys.stdout.write("\n\nBlending lightsheets and moving to final dst...")
        sys.stdout.flush()
        for dst, flds in outdct.items():
            if len(flds) == 2:
                blend_lightsheets(flds, dst, cores)
            if len(flds) == 1:
                blend_lightsheets([flds[0], flds[0]], dst, cores)  # simulating two
        sys.stdout.write("...completed")
        sys.stdout.flush()
...
def make_jobs(image_dictionary, jobid=False, raw_data_src=False):
    ...
    if type(jobid) == int:
        volumes = [image_dictionary["volumes"][jobid]]
    elif not jobid:
        volumes = image_dictionary["volumes"]
    ...
    for volume in volumes:
        for lightsheet in range(volume.lightsheets):
            ...
            if isinstance(raw_data_src, str):  # for rat brain/low res images
                ...
                zdct = {}
                [zdct.update(dict([(keys, dict([(k, v)]))]))
                 for keys, values in tmpdct["zchanneldct"].items() for k, v in values.items()]
            ...
            if volume.lightsheets == 2:
                ...
                ls = lslst[lightsheet]
                zdct = {k: {kk: [xx for xx in vv if side in xx]}
                        for k, v in zdct.items() for kk, vv in v.items()}
            ...
            final_dst = volume.full_sizedatafld_vol
            makedir(final_dst)
            tmp_dst = os.path.join(os.path.dirname(list(list(volume.zdct.values())[0].values())[0][0]),
                                   os.path.basename(volume.full_sizedatafld_vol)+"_"+ls)
            makedir(os.path.dirname(tmp_dst))
            makedir(tmp_dst)
            ts_out = os.path.join(os.path.dirname(list(list(volume.zdct.values())[0].values())[0][0]),
                                  os.path.basename(volume.full_sizedatafld_vol)+"_"+ls+"_ts_out")
            makedir(ts_out)
...
def blend_lightsheets_pre_stitching(left, right, dst, cores):
    ...
    iterlst = [{"xdim": xdim0, "ydim": ydim0, "fl0": left[i], "channel": ch, "fl1":right[i],
                "dst":dst, "name":name, "zplane":i, "stitched":False} for i, l in enumerate(left)]
...
def blend(dct):
    ...
    fl0 = dct["fl0"]
    fl1 = dct["fl1"]
    alpha = np.tile(scipy.stats.logistic.cdf(
        np.linspace(-4, 4, num=dct["xdim"])), (dct["ydim"], 1))  # linspace was -275,275
    im0 = sitk.GetArrayFromImage(sitk.ReadImage(fl0))
    dtype = im0.dtype
    im1 = sitk.GetArrayFromImage(sitk.ReadImage(fl1))
    ch = "_C"+dct["channel"]
    im0, im1 = pixel_shift(im0, im1)
    stitched = dct["stitched"] if "stitched" in dct else True
    if stitched:
        im_final_shape = (im0.shape[1] if im0.shape[1] < im1.shape[1] else im1.shape[1], im0.shape[0]
                          if im0.shape[0] < im1.shape[0] else im1.shape[0])  # width, height format for cv2
        im0_resz = cv2.resize(im0, im_final_shape)
        im1_resz = cv2.resize(im1, im_final_shape)
        tifffile.imsave(os.path.join(dct["dst"], dct["name"]+ch+"_Z"+str(dct["zplane"]).zfill(
            4)+".tif"), (im0_resz*(1-alpha) + im1_resz * (alpha)).astype(dtype), compress=1)
    else:
        tifffile.imsave(os.path.join(dct["dst"], os.path.basename(fl0)),
                        (im0*(1-alpha) + im1 * (alpha)).astype(dtype), compress=1)
    return
...
def terastitcher_par(inndct):
    ...
    image_dictionary = inndct["dct"]
    tmp_dst = inndct["tmp_dst"]
    channel = inndct["channel"]
    lightsheet = inndct["lightsheet"]
    transfertype = inndct["transfertype"]
    voxel_size = inndct["scalefactor"]
    percent_overlap = inndct["percent_overlap"]
    algorithm = inndct["algorithm"]
    outbitdepth = inndct["outbitdepth"]
    threshold = inndct["threshold"]
    cores = 1
...
def call_terastitcher(src, dst, voxel_size, threshold, algorithm="MIPNCC", outbitdepth="16", resolutions="0"):
    ...
    st = time.time()
    # import
    # make sure its rounded (fix for 1.1x images)
    voxel_size = tuple(map(lambda x: isinstance(x, float) and round(x, 2) or x, voxel_size))

    sys.stdout.write("\n\nRunning Terastitcher import on {}....".format(
        " ".join(src.split("/")[-2:])))
    sys.stdout.flush()
    ...
    # align
    sys.stdout.write("\n\nRunning Terastitcher alignment on {}, this can take some time....".format(
        " ".join(src.split("/")[-2:])))
    sys.stdout.flush()
    xml_displcomp = os.path.join(src, "xml_displcomp.xml")
    call1 = "terastitcher --displcompute --projin={} --projout={} --sV=100 --sH=100 --sD=10".format(
        xml_import, xml_displcomp)
    sp_call(call1)
    sys.stdout.write("\n...completed!")
    sys.stdout.flush()
    ...
    call3 = "terastitcher --displthres --projin={} --projout={} --threshold={}".format(
        xml_displproj, xml_displthresh, threshold)
    sp_call(call3)

    # placetiles
    xml_placetiles = os.path.join(src, "xml_placetiles.xml")
    call4 = "terastitcher --placetiles --projin={} --projout={} --algorithm=MST".format(
        xml_displthresh, xml_placetiles)
    sp_call(call4)

    # merge
    sys.stdout.write("\nOutputting images, this can also take some time....")
    sys.stdout.flush()
    makedir(dst)
...
def make_folder_heirarchy(image_dictionary, dst, channel, lightsheet=False,
    ...
    # image dims
    im = tifffile.imread(pth_update(
        list(list(image_dictionary["zchanneldct"].values())[0].values())[0][0]), multifile=False)
    if im.ndim == 3:
        sys.stdout.write("flattening (maxip) multiplane tiffs")
        sys.stdout.flush()
        hf, ypx, xpx = im.shape
        iterlst = [zz for xx in list(image_dictionary["zchanneldct"].values())
                   for yy in list(xx.values()) for zz in yy]
        if cores >= 2:
            p = mp.Pool(cores)
        ...
        sys.stdout.write("completed flattening (maxip) multiplane tiffs")
        sys.stdout.flush()
    ...
    for y in range(ytile):
        ...
        for x in range(xtile):
            ...
            for z in image_dictionary["zchanneldct"]:
                ...
                lst = image_dictionary["zchanneldct"][str(z).zfill(4)][channel]
                lst.sort()
    ...
    if cores >= 2:
        sys.stdout.write(
            " populating folders: {} files using {} cores...\n".format(len(iterlst), cores))
        sys.stdout.flush()
        p = mp.Pool(cores)
        ...
        sys.stdout.write(" populating folders...")
        sys.stdout.flush()
...
def make_folder_heirarchy_helper(src, dst, transfertype):
    ...
    """
    import shutil
    if transfertype == "move" and not os.path.exists(dst):
        shutil.move(src, dst)
    elif transfertype == "copy" and not os.path.exists(dst):
        shutil.copy(src, dst)
...
def blend_lightsheets(flds, dst, cores, cleanup=False):
    ...
    iterlst = [{"xdim": xdim, "ydim": ydim, "fl0": fls0[i], "channel": ch,
                "fl1":fls1[i], "dst":dst, "name":name, "zplane":i} for i, fl0 in enumerate(fls)]
...
def pixel_shift(a, b):
    ...
    if am > bm:
        a = (a - delta).clip(min=0).astype(dtype)
    if bm > am:
        b = (b - delta).clip(min=0).astype(dtype)
    return a, b
...
def resize(src, resizefactor, cores):
    ...
    # set up dsts
    [makedir(os.path.join(os.path.dirname(src), xx[:-4]+"resized_"+xx[-4:]))

...
def generate_transformed_cellcount(dataframe, dst, transformfiles, lightsheet_parameter_dictionary, verbose=False):
    ...
    fullsizedimensions = get_fullsizedims_from_kwargs(kwargs)
...
if __name__ == '__main__':
    ...
    dataframe = pd.read_csv(
        '/home/wanglab/wang/pisano/tracing_output/antero_4x/20170115_tp_bl6_lob6a_1000r_02/3dunet_output/pooled_cell_measures/20170115_tp_bl6_lob6a_1000r_02_cell_measures.csv')
    ...
    converted_points = generate_transformed_cellcount(
        dataframe, dst, transformfiles, lightsheet_parameter_dictionary, verbose=verbose)

    # load and convert to single voxel loc
    zyx = load_np(converted_points)
    zyx = np.asarray([str((int(xx[0]), int(xx[1]), int(xx[2])))
